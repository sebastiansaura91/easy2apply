  ...
  for (const section of enabledSections) {
    switch (section.type) {
      ...
      case "skills": {
        ...
        // One comma-separated paragraph (mirrors the preview): the densest honest
        // format, ~4 lines instead of a bulleted three-column grid, ATS-identical.
        const lineH = 10 * 1.4 * 0.3528;
        pdf.setFontSize(10);
        pdf.setFont(font, "normal");
        pdf.setTextColor(...colors.black);
        const lines = pdf.splitTextToSize(cv.skills.filter(Boolean).join(", "), contentW);
        checkPage(lines.length * lineH + 1);
        for (const ln of lines) {
          pdf.text(ln, marginL, y);
          y += lineH;
        }
        y += 1;
        break;
